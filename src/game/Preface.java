...
class Preface extends JFrame {
	...
	private void initInterface() {
		...
		layeredBackgroundLabel = new JLabel(new ImageIcon(
				"./images/snowyNight.gif"));// 设置背景图片
		layeredBackgroundLabel.setBounds(0, 0, layeredBackgroundLabel.getIcon()
				.getIconWidth(), layeredBackgroundLabel.getIcon()
				.getIconHeight());
		// 将含背景图片的标签放在RootPane层
		getLayeredPane().add(layeredBackgroundLabel,
		...
		panel.setLocation(
				(layeredBackgroundLabel.getIcon().getIconWidth() - newGameButton
						.getIcon().getIconWidth()) / 2,
				(layeredBackgroundLabel.getIcon().getIconHeight() - 4
						* newGameButton.getIcon().getIconHeight() - 10 * 3) * 3 / 4);
		panel.setSize(newGameButton.getIcon().getIconWidth(), 4 * newGameButton
				.getIcon().getIconHeight() + 10 * 3);
		add(panel);
		...
		setSize(layeredBackgroundLabel.getIcon().getIconWidth()
				+ getInsets().left + getInsets().right, layeredBackgroundLabel
	...
	private void myEvent() {
		...
		addWindowListener(new WindowAdapter() {
			public void windowDeactivated(WindowEvent e) {
				layeredBackgroundLabel.setIcon(new ImageIcon(
						"./images/snowyNightStop.png"));
		...
		continueGameButton.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				if (!GameBase.isNewGame) {
					...
					JOptionPane.showMessageDialog(Preface.this, "没有可继续的游戏",
