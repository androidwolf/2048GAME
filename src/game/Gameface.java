...
class Gameface extends JFrame {
	...
	private void initGameface() {
		JLabel scoreBoardLabel = new JLabel(new ImageIcon(
				"./images/scorebg.png"));
		scoreBoardLabel.setBounds(0, 0, scoreBoardLabel.getIcon()
				.getIconWidth(), scoreBoardLabel.getIcon().getIconHeight());
		...
		rootBackgroundLabel.setBounds(scorePanel.getWidth(), 0,
				rootBackgroundLabel.getIcon().getIconWidth(),
				rootBackgroundLabel.getIcon().getIconHeight());
		// 将含背景图片的标签放在RootPane层
		getRootPane().add(rootBackgroundLabel, new Integer(Integer.MIN_VALUE));

		layeredBackgroundLabel1 = new JLabel(new ImageIcon(
				"./images/runningConan.gif"));// 设置背景图片
		layeredBackgroundLabel1.setBounds(75, 88, layeredBackgroundLabel1
				.getIcon().getIconWidth(), layeredBackgroundLabel1.getIcon()
				.getIconHeight());
		layeredBackgroundLabel2 = new JLabel(new ImageIcon(
				"./images/centerSpinning2048.gif"));// 设置背景图片
		layeredBackgroundLabel2.setBounds(0, 0, layeredBackgroundLabel2
				.getIcon().getIconWidth(), layeredBackgroundLabel2.getIcon()
				.getIconHeight());
		visualPanel = new JPanel(null);
		visualPanel.setBounds(scorePanel.getWidth(), 0, layeredBackgroundLabel2
				.getIcon().getIconWidth(), layeredBackgroundLabel2.getIcon()
				.getIconHeight());
		...
		JLabel connectLabel = new JLabel("  欢迎加我QQ与我交流：598515834");
		...
		connectLabel.setBounds(0, 130 + buttonsPanel.getHeight(),
				visualPanel.getWidth(), 20);
		...
		cardsPanel.setBounds(0, scoreBoardLabel.getIcon().getIconHeight(),
				scoreBoardLabel.getIcon().getIconWidth(), scoreBoardLabel
		...
		setSize(800 + getInsets().left + getInsets().right, 600
	...
	private void setVisualOff() {
		...
		layeredBackgroundLabel1.setIcon(new ImageIcon(
				"./images/runningConanStop.png"));
		layeredBackgroundLabel2.setIcon(new ImageIcon(
				"./images/centerSpinning2048Stop.png"));
	}
	...
	private void moveCards(int direction) {
		...
		if (isMovable(presentCardsNumber, direction)) {
			...
			createCard();
			timer = new Timer();
			timer.schedule(new GameEndTask(), 0);
		}
	}
	...
	class GameEndTask extends TimerTask {
		public void run() {
			...
			gameEndLabel.setBounds(50, 150, gameEndLabel.getIcon()
					.getIconWidth(), gameEndLabel.getIcon().getIconHeight());
	...
	private void createCard() {
		...
		if ((int) (100 * Math.random() % 9) == 0)
			...
			cardsNumber[x][y] = 2;
		cards[x][y].setIcon(new ImageIcon("./images/" + cardsNumber[x][y]
				+ "-pre.gif"));
		timer = new Timer();
	...
	private void displayCards() {
		for (int i = 0; i <= 3; i++) {
			for (int j = 0; j <= 3; j++) {
				cards[i][j].setIcon(new ImageIcon("./images/"
						+ cardsNumber[i][j] + ".png"));
			}
		}
