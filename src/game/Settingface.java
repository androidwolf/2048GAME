...
class Settingface extends JDialog {
	...
	private static JComboBox soundsComboBox;
	...
	private void initSettingface() {
		...
		soundsComboBox = new JComboBox(
				new DefaultComboBoxModel(GameBase.SOUNDS));
		...
		soundsPanel.setBorder(new TitledBorder(null, "音视效果设置",
				TitledBorder.DEFAULT_JUSTIFICATION,
				TitledBorder.DEFAULT_POSITION, null, Color.BLUE));

		lookAndFeelLabel = new JLabel("选择风格：");
		lookAndFeelComboBox = new JComboBox(new DefaultComboBoxModel(
				GameBase.STYLES));
		...
		lookAndFeelPanel.setBorder(new TitledBorder(null, "窗口风格设置",
				TitledBorder.DEFAULT_JUSTIFICATION,
				TitledBorder.DEFAULT_POSITION, null, Color.BLUE));
	...
	private void myEvent() {
		addWindowListener(new WindowAdapter() {
			public void windowClosing(WindowEvent e) {
				int select = JOptionPane.showConfirmDialog(Settingface.this,
						"是否保存设置？", "提示", JOptionPane.YES_NO_OPTION);
				if (0 == select)
					save();
			}
		});
