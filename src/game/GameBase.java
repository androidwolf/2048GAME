...
class GameBase {
	...
	static final String[] SOUNDS = { "sound1.mid", "sound2.mid", "sound3.mid",
			"sound4.mid", "sound5.mid", "sound6.mid" };
	...
	static AudioClip getSound(String sound) {
		try {
			...
			int select = JOptionPane.showConfirmDialog(null, "背景音乐加载失败，是否继续？",
	...
	static void setLookAndFeel(Component c, String style) {
		try {
			...
			JOptionPane.showMessageDialog(c, "设置窗口风格失败", "错误提示",
	...
	static GameData readMyData() {
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "游戏加载失败", "错误提示",
	...
	static void writeMyData(GameData data) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "数据写入发生异常", "错误提示",
					JOptionPane.ERROR_MESSAGE);
		} finally {
			try {
				if (fw != null)
